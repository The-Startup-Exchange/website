        ...
        <>
            ...
            <div className={`flex flex-col w-full items-center pb-12 justify-center ${theme === 'dark' ? 'text-white bg-black' : 'text-black bg-white'}`}>
                <div className={`flex flex-col w-[94%] py-32 px-12 ${theme === 'dark' ? 'border-[#242424]' : 'border-[#E0E0E0]'} border-x items-start justify-center gap-y-4`}>
                    <h1 className={`w-full text-start text-[21px] pb-16 leading-[125%] ${messina_book.className}`}>CHAPTERS</h1>
                ...
                {Array.from({ length: numRows }, (_, rowIndex) => (
                    <div className={`flex flex-row w-[94%] p-20 ${theme === 'dark' ? 'border-[#242424]' : 'border-[#E0E0E0]'} border-x justify-between gap-x-16`}>
                        {campuses.slice(rowIndex * 2, (rowIndex + 1) * 2).map((campus) => (
                            <Link href={campus.link} className={`flex flex-col align-start space-y-6 text-start ${plus_jakarta_sans_regular.className}`}>
                                <div className="relative w-full h-full">
                                    <Image src={campus.pictureName} alt={campus.name} width={560} height={448}/>
                                    <div className="absolute inset-0 bg-gray-500 opacity-0 hover:opacity-60 flex items-center justify-center transition-opacity duration-300">
